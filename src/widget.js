...
(function () {
  ...
  var STYLES =
    ...
    '.aisw[data-color-theme="dark"]{color:#e8e8e8}' +
    ...
    '.aisw[data-color-theme="dark"] .aisw_b{color:#999}' +
    '.aisw[data-color-theme="dark"] .aisw_b:hover{background:rgba(255,255,255,.08)}' +
    ...
    '.aisw[data-color-theme="dark"] .aisw_b.aisw_b_color.aisw_b_chatgpt svg,.aisw[data-color-theme="dark"] .aisw_b.aisw_b_color.aisw_b_grok svg{filter:brightness(0) invert(1) opacity(.95)}' +
    '.aisw[data-icon-size="s"] .aisw_b{width:32px;height:32px}.aisw[data-icon-size="s"] .aisw_b svg{width:18px;height:18px}' +
    '.aisw[data-icon-size="l"] .aisw_b{width:44px;height:44px}.aisw[data-icon-size="l"] .aisw_b svg{width:26px;height:26px}';
  ...
  // Walk up the DOM until we find an opaque background, then decide light/dark
  // by perceptual luminance. Used for data-theme="auto".
  ...
  function render(cfg) {
    try {
      ...
      // Determine where the widget will mount BEFORE detecting theme
      var mountParent;
      var insertBeforeNode = null;
      if (cfg.target) {
        mountParent = document.querySelector(cfg.target) || document.body || document.documentElement;
      } else {
        var s = SELF || findScript();
        if (s && s.parentNode && s.parentNode.tagName !== 'HEAD') {
          mountParent = s.parentNode;
          insertBeforeNode = s.nextSibling;
        } else {
          mountParent = document.body || document.documentElement;
        }
      }

      var root = document.createElement('div');
      root.className = 'aisw';
      // Auto-detect theme from the actual mount-parent background
      var theme = cfg.theme || 'auto';
      if (theme === 'auto') theme = detectTheme(mountParent);
      root.setAttribute('data-color-theme', theme);
      if (cfg.size) root.setAttribute('data-icon-size', cfg.size);
      ...
      if (insertBeforeNode !== null) {
        mountParent.insertBefore(root, insertBeforeNode);
      } else {
  ...
  function init() {
    ...
    render({
      ...
      label: d.label,
      theme: d.colorTheme,
      size: d.iconSize === 'sm' ? 's' : d.iconSize === 'lg' ? 'l' : null,
